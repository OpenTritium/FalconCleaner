...
/// </summary>
internal class PhysicalDiskGpt
{
	...
	internal PhysicalDiskGpt(ulong id, SafeFileHandle handle)
	{
		Id = id;
		Handle = handle;
		...
		unsafe
		{
			...
			bool TryGetDriveLayout()
			{
				try
				{
					return DeviceIoControl(Handle, IoControlCodes.GetDriveLayoutEx, IntPtr.Zero, 0,
						new IntPtr(bufferPointer), presetSize, out _, ref NullNativeOverlapped)
						? true
			...
			{
				var name = Marshal.PtrToStringUni((IntPtr)info.DummyUnionName.Gpt.Name, 36);
			...
			Console.WriteLine(DiskSize.ToString());
			Geometry = diskGeometryExPointer->geometry;
			Console.WriteLine(Geometry.BytesPerSector);
			NativeMemory.Free(bufferPointer);
		}
	}
}